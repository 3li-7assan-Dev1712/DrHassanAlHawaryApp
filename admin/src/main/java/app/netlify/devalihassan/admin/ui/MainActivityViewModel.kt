package app.netlify.devalihassan.admin.ui
...
class MainActivityViewModel @Inject constructor(
    ...
    private suspend fun storeUserDataSuspend() {
        Log.d(TAG, "storeUserDataSuspend: 1")
        val studentData = getStudentDataUseCase().first()
        if (studentData == null) {
            Log.d(TAG, "storeUserDataSuspend:student data: $studentData")
            val userData = getCurrentUserDataUseCase()
            val userSecurityRule = checkIfUserIsAdminUseCase()
            Log.d(TAG, "storeUserDataSuspend: role: $userSecurityRule")
    ...
    fun checkUserAuthState() {
        viewModelScope.launch {
            ...
            if (isLoggedIn) {
                ...
                Log.d(TAG, "checkUserAuthState: id Token : $idToken")
                _state.update {
                    it.copy(
